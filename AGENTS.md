...
The directory below is an opaque operator-secret boundary:

`%USERPROFILE%\.kinotch-secrets\`

Agents MUST NOT:

- read files under this directory
- list directory contents
- search the directory
- inspect file metadata unless required only to determine existence
- print or dump files from the directory
- use `Get-Content`, `cat`, `type`, `more`, `Select-String`, `findstr`, Python, Node, PowerShell, or another tool to inspect secret contents
- print related environment variable values
- dump `process.env`
- copy secret values into repository files, logs, prompts, patches, comments, issues, commits, or responses
- modify or delete secret files

Agents MAY:

- invoke the repository-provided secret mapper
- execute `npm run smoke:mcp:local`
- execute `npm run release:local`

The mapper must be treated as an opaque secret-input boundary. If a secret is missing or invalid, report only the missing key name or the safe error emitted by the mapper/release gate. Never request that the operator paste a secret value into chat.
