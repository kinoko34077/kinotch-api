...
- Pull requests that change the root `package.json` or `package-lock.json` trigger the repository-local `Dependency Review` workflow with `contents: read`, vulnerability checking at `low` severity or above, license checking disabled, and immutable Action commit pins. Live Dependabot PR #37 proved the trigger path executes, but GitHub currently rejects the review step because Dependency graph is disabled for this repository; until that repository-admin/security setting is explicitly enabled, this workflow does not provide dependency-delta vulnerability coverage. It is not a protected required check, and CodeQL remains deferred.
